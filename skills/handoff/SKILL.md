...
2. **Every gate gets a ruling, not a skip.** When a superpowers skill says "get approval", "ask your human partner", "STOP and wait", or "Which option?", do the work that would have produced the question, then dispatch `handoff-proxy` with a gate packet, apply the ruling it returns, and log it. Do not silently pass through a gate. Deciding a DECIDE gate yourself is allowed only under the same-model rule in "Who decides" below, and it still gets the same evidence gathering and the same logged ruling line.
